...
def cria(client, headers, room=None):
    return client.post("/rooms", json=room or SALA_ACESSIVEL, headers=headers)
...
class TestMarcacao:
    ...
    def test_poltrona_comum_nao_vira_registro(self, client):
        ...
        assert sector["capacity"] == 72
        assert len(sector["special_seats"]) == 4
    ...
    def test_setor_sem_marcacao_fica_vazio(self, client):
        ...
        r = cria(client, auth(client, ORGANIZADOR), room)
        assert r.status_code == 201
...
class TestValidacao:
    @pytest.mark.parametrize("code", ["G1", "A13", "Z9", "A0"])
    def test_poltrona_fora_da_geometria_e_recusada(self, client, code):
            ...
            "sectors": [
                {
                    ...
                    "rows": 6,
                    "seats_per_row": 12,
                    "special_seats": [{"seat_code": code, "kind": "WHEELCHAIR"}],
                }
            ],
        }
        r = cria(client, auth(client, ORGANIZADOR), room)
    ...
    def test_mesma_poltrona_marcada_duas_vezes_e_recusada(self, client):
        ...
        assert cria(client, auth(client, ORGANIZADOR), room).status_code == 422
...
class TestVisibilidadeNaVitrine:
    def test_detalhe_publico_expoe_as_poltronas_acessiveis(self, client):
        ...
        publico = client.get(f"/sessions/{session['id']}")
