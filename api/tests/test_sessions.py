...
def futuro(days: int = 2, hora: int = 20) -> str:
    starts_at = datetime.now(timezone.utc) + timedelta(days=days)
    return starts_at.replace(hour=hora, minute=0, second=0, microsecond=0).isoformat()
...
def payload_sessao(room: dict, **extra) -> dict:
    return {
        "catalog_id": um_filme(),
        "room_id": room["id"],
        "starts_at": futuro(),
...
class TestCriacao:
    def test_cria_sessao_como_rascunho(self, client):
        ...
        r = client.post("/organizer/sessions", json=payload_sessao(room), headers=headers)
    ...
    def test_cria_ja_publicada(self, client):
        ...
        room = cria_sala(client, headers)
        r = client.post(
            "/organizer/sessions", json=payload_sessao(room, publish=True), headers=headers
        )
    ...
    def test_guarda_copia_dos_dados_do_filme(self, client):
        ...
        assert movie["title"] == esperado.title
        assert movie["poster_url"] == esperado.poster_url
        assert movie["runtime_minutes"] == esperado.runtime_minutes
    ...
    def test_faixa_de_precos(self, client):
        ...
        room = cria_sala(client, headers)
        corpo = client.post(
            "/organizer/sessions", json=payload_sessao(room), headers=headers
        ).json()
...
class TestValidacoes:
    def test_horario_no_passado_e_recusado(self, client):
        ...
        r = client.post(
            "/organizer/sessions",
            json=payload_sessao(room, starts_at=passado),
            headers=headers,
        )
    ...
    def test_horario_sem_fuso_e_recusado(self, client):
        ...
        r = client.post(
            "/organizer/sessions",
            json=payload_sessao(room, starts_at="2027-01-01T20:00:00"),
            headers=headers,
        )
        assert r.status_code == 422
    ...
    def test_filme_inexistente_e_recusado(self, client):
        ...
        r = client.post(
            "/organizer/sessions",
            json=payload_sessao(room, catalog_id="000000"),
            headers=headers,
        )
...
class TestCicloDeVida:
    def test_publicar_e_despublicar(self, client):
        ...
        publicada = client.post(f"/organizer/sessions/{session['id']}/publish", headers=headers)
        assert publicada.json()["status"] == "PUBLISHED"

        oculta = client.post(f"/organizer/sessions/{session['id']}/unpublish", headers=headers)
    ...
    def test_sessao_cancelada_nao_volta_atras(self, client):
        ...
        r = client.post(f"/organizer/sessions/{session['id']}/publish", headers=headers)
        assert r.status_code == 409
    ...
    def test_atualiza_precos(self, client):
        ...
        novos = [{"sector_id": s["id"], "price_cents": 9900} for s in room["sectors"]]
        r = client.patch(
            f"/organizer/sessions/{session['id']}", json={"prices": novos}, headers=headers
        )
...
class TestVitrinePublica:
    ...
    def test_rascunho_nao_aparece(self, client):
        headers = auth(client, ORGANIZADOR)
        room = cria_sala(client, headers)
        client.post("/organizer/sessions", json=payload_sessao(room), headers=headers)
    ...
    def test_busca_por_titulo(self, client):
        ...
        assert client.get("/sessions", params={"search": titulo[:6]}).json()["total"] == 1
        assert client.get("/sessions", params={"search": "zzzznada"}).json()["total"] == 0
